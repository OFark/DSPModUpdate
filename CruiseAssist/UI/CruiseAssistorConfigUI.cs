...
namespace tanu.CruiseAssist
{
	public class CruiseAssistorConfigUI
	{
		...
		public static void OnGUI()
		{
			...
			Rect[wIdx] = GUILayout.Window(99030293, Rect[wIdx], WindowFunction, "Cruise Assistor - Config", windowStyle);
		...
		public static void WindowFunction(int windowId)
		{
			...
			int listSelected =
				CruiseAssistorMainUI.ViewMode == CruiseAssistMainUIViewMode.FULL ? 0 : 1;
			GUI.changed = false;
			...
			GUI.changed = false;
			CruiseAssistor.MarkVisitedFlag = GUILayout.Toggle(CruiseAssistor.MarkVisitedFlag, "Mark the visited system and planet.", toggleStyle);
			if (GUI.changed)
			{
				VFAudio.Create("ui-click-0", null, Vector3.zero, true, 0);
				CruiseAssistorMainUI.NextCheckGameTick = GameMain.gameTick + 300;
			...
			GUI.changed = false;
			CruiseAssistor.SelectFocusFlag = GUILayout.Toggle(CruiseAssistor.SelectFocusFlag, "Focus when target selected.", toggleStyle);
			if (GUI.changed)
			{
				VFAudio.Create("ui-click-0", null, Vector3.zero, true, 0);
				CruiseAssistorMainUI.NextCheckGameTick = GameMain.gameTick + 300;
			...
			GUI.changed = false;
			CruiseAssistor.HideDuplicateHistoryFlag = GUILayout.Toggle(CruiseAssistor.HideDuplicateHistoryFlag, "Hide duplicate history.", toggleStyle);
			if (GUI.changed)
			{
				VFAudio.Create("ui-click-0", null, Vector3.zero, true, 0);
				CruiseAssistorMainUI.NextCheckGameTick = GameMain.gameTick + 300;
			...
			GUI.changed = false;
			CruiseAssistor.AutoDisableLockCursorFlag = GUILayout.Toggle(CruiseAssistor.AutoDisableLockCursorFlag, "Disable lock cursor when starting sail mode.", toggleStyle);
			if (GUI.changed)
			{
				VFAudio.Create("ui-click-0", null, Vector3.zero, true, 0);
				CruiseAssistorMainUI.NextCheckGameTick = GameMain.gameTick + 300;
